...
for episode in range(1, number_episodes + 1):
  ...
  for t in range(maximum_number_timesteps_per_episode):
    action = agent.act(state, epsilon)                  # select an action for given state of env
    next_state, reward, done, _, _ = env.step(action)
    agent.step(state, action, reward, next_state, done)  # store the experience in replay memory and learn from it
    state = next_state
    score += reward
    if done:
      break
